...
class GoogleSignInPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    /**
     * Prefer the explicit Sign in with Google button flow.
     *
     * GetGoogleIdOption alone often fails with NoCredentialException (or
     * TransactionTooLargeException on Android 14+) when many Google accounts
     * are on the device — even though accounts exist. GetSignInWithGoogleOption
    ...
    private suspend fun requestGoogleIdToken(clientId: String): String {
        try {
            ...
            Log.w(
                TAG,
                "GetSignInWithGoogleOption failed (${e.type}), falling back to GetGoogleIdOption",
                e
            )
            return getCredentialIdToken(
